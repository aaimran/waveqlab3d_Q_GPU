...
contains
  subroutine try_traditional_lx_boundary(B, mms_vars, handled)
    ...
    integer :: mx, my, mz, py, pz, ix, n1, n2, n3
    ...
    if (B%boundary_vars%Lx < 1 .or. B%boundary_vars%Lx > 3) return
    if (B%boundary_vars%Rx > 0 .or. B%boundary_vars%Ly > 0 .or. &
        B%boundary_vars%Ry > 0 .or. B%boundary_vars%Lz > 0 .or. &
    ...
    if (.not.acc_is_present(B%F%F,field_bytes) .or. &
        ...
        .not.acc_is_present(B%B(1)%n_l,face_bytes) .or. &
        .not.acc_is_present(B%B(1)%n_m,face_bytes) .or. &
        .not.acc_is_present(B%B(1)%n_n,face_bytes) .or. &
    ...
    if (environment_true('WQL3D_PHASE6_DIAGNOSTICS') .and. .not.launch_reported) then
      write(*,'(A,I0,A,I0,A,I0)') 'Phase 6 Lx launch: ny=',py-my+1, &
           ' nz=',pz-mz+1,' bc=',B%boundary_vars%Lx
  ...
  subroutine lx_boundary_kernel(field,rate,material,metricx,nl,nm,nn,work, &
    ...
    do k=1,nz
      do j=1,ny
        ...
        ubc(4)=-norm*nv(1)*f_x; ubc(5)=-norm*nv(2)*f_y; ubc(6)=-norm*nv(3)*f_z
        ubc(7)=-norm*(nv(2)*f_x+nv(1)*f_y)
        ubc(8)=-norm*(nv(3)*f_x+nv(1)*f_z)
        ubc(9)=-norm*(nv(3)*f_y+nv(2)*f_z)
        work(j,k,:)=ubc
        ux(1:3)=ubc(1:3)/rho
  ...
  end subroutine lx_boundary_kernel
